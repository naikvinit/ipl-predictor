...
def parse_csv(file, expected_cols):
    ...
    for col in expected_cols:
        if isinstance(col, (list, tuple, set)):
            normalized_expectations.append(tuple(col))
        else:
            normalized_expectations.append((col,))
    missing = []
    for options in normalized_expectations:
        if not any(opt in fieldnames for opt in options):
            missing.append("/".join(options))
    if missing:
        st.error(f"CSV missing columns: {missing}. Found columns: {fieldnames}")
        return None
...
def fixtures_ui():
    with st.expander("📥 Upload Fixtures", expanded=False):
        st.caption(
            "CSV must include: match_id (int), match_date or date (ISO), team_a, team_b, week (int), "
            "time_ist (string), venue (string). Optional columns like day will also be stored."
        )
        ...
        if f and st.button("Import Fixtures", type="primary"):
            ...
            if rows is not None:
                payload = []
                for r in rows:
                    match_date = r.get("match_date") or r.get("date")
                    if not match_date:
                        st.error("Each row must include match_date or date.")
                        payload = []
                        break
                    payload.append({
...
def deadline_ui():
    with st.expander("⏱️ Prediction Deadline", expanded=False):
        st.caption("Control when predictions become locked across the app.")
        current_value = db.get_actual_meta(DEFAULT_CUTOFF_KEY) or CUTOFF_ISO
        try:
            parsed = _parse_cutoff_iso(current_value)
            current_display = parsed.strftime("%Y-%m-%d %H:%M:%S %Z")
        except Exception:
            parsed = None
            current_display = f"Invalid value stored: {current_value}"
        ...
        if st.button("Save new cutoff", type="primary", key="save_cutoff_btn"):
            try:
                parsed = _parse_cutoff_iso(new_value.strip())
                db.set_actual_meta(DEFAULT_CUTOFF_KEY, new_value.strip())
                st.success(
                    "Updated prediction deadline to "
                    + parsed.strftime("%Y-%m-%d %H:%M:%S %Z")
                )
            except Exception as exc:
                st.error(f"Unable to save cutoff: {exc}")
